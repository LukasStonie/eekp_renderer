    ...
    <main class="container md:mx-auto px-8 xl:px-4 max-w-6xl flex-grow">
      ...
      <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-6 mt-4 w-full">
        ...
        <div class="flex gap-2 w-full lg:max-w-md">
          <el-input v-model="identifier" placeholder="Nach Identifier suchen" size="large" class="flex-1" clearable />
        ...
        <div class="flex justify-start lg:justify-end">
          <el-dropdown trigger="click" class="w-full lg:w-auto">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item v-for="q in questionnaires.filter(x => x.render == true)" :key="q.url"
      ...
      <div class="mt-10 overflow-x-auto">
        <div v-if="questionnaireResponses && questionnaireResponses.length > 0">
          <transition name="el-zoom-in-top" appear>
            <el-table v-loading="loading" :data="questionnaireResponses" row-key="resource.authored" style="width: 100%"
              class="rounded-lg shadow-sm">
              <el-table-column label="Identifier" min-width="180">
                <template #default="{ row }">
                  <span v-if="row.resource?.identifier?.value || row.resource?.identifier?.[0]?.value"
                    class="text-gray-700 font-mono">
                    {{ row.resource.identifier.value || row.resource.identifier[0].value }}
                  </span>
              ...
              <el-table-column label="Aktionen" width="120" fixed="right">
                <template #default="{ row }">
                  <el-button size="small" color="#FBEF7A" class="font-semibold"
                    :disabled="!(row.resource?.identifier?.value || row.resource?.identifier?.[0]?.value)"
        ...
        <div v-else class="py-20 flex justify-center">
          <el-alert title="Keine Elemente gefunden"
            description="Passen Sie den Identifier an oder legen Sie einen neuen Eintrag an." center show-icon
            type="info" :closable="false" class="max-w-xl" />
        </div>
      </div>
    </main>
  </div>
...
onMounted(async () => {
  ...
  try {
    const data: any = await $fetch('/api/questionnaires/read', {
      method: 'GET',
      params: { _count: 10, _sort: `-authored` }
    })
...
const searchForIdentifier = async (id: string) => {
  try {
    ...
    const data: any = await $fetch('/api/questionnaires/read', {
      method: 'GET',
      params: {
        identifier: `${config.public.defaultQuestionnaireIdentifierSystem}|${id}`,
        _count: 10,
        _sort: "-authored"
      }
    })
