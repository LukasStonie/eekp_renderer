...
export default defineEventHandler(async (event) => {
  ...
  try {
    const response = await axios.post(config.hcpEntryPoint, xmlBody, {
      httpsAgent,
      headers: {
    ...
    if (!xmlText.includes("Assertion") || !xmlText.includes("PUBLICHEALTH")) {
      throw createError({
        statusCode: 500,
        statusMessage: "Required HCP Assertion data missing in response"
      });
    }
    ...
    if (startIndex === -1 || endIndex === -1) {
      throw new Error("SAML Boundary not found");
    }
    const hcpGda = xmlText.substring(startIndex, endIndex);
    
    const idMatch = hcpGda.match(/ID="([^"]+)"/);
    const hcpGdaAid = idMatch ? idMatch[1] : null;

    const hcpGdaB64 = Buffer.from(hcpGda).toString('base64');
    return {
      success: true,
      hcpGda: hcpGda,
      hcpGdaB64: hcpGdaB64,
    ...
    throw createError({
      ...
      statusMessage: 'Failed to perform HCP request'
    })
  }
})


export const createSoapRequest = (params: { guid: string, ctxID: string, idaGda?: string }): string => {
  return `
  ...
  <env:Body>
    <RequestSecurityToken xmlns="http://docs.oasis-open.org/ws-sx/ws-trust/200512">
      ...
      <Claims Dialect="urn:tiani-spirit:bes:2013:claims">
        <ClaimType xmlns="urn:tiani-spirit:ts" DataType="http://www.w3.org/2001/XMLSchema#anyURI" name="urn:tiani-spirit:bes:2013:claims:requested-role">
